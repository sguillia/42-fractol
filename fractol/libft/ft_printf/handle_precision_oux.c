...
void	handle_precision_oux(t_options *options, t_list *output, char *str)
{
	int		len;
	char	*ret;
	char	*zeros;
	...
	if (options->precision == 0 && *str == '0')
	{
		...
		free(str);
		handle_converted_oux(options, output, ret);
		return ;
	}
	if (options->precision == -1 || len >= options->precision)
	{
		handle_converted_oux(options, output, str);
		return ;
	}
