...
/*   Updated: 2016/02/12 22:04:32 by sguillia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	end_ft_printf(t_list *output, t_list *args, char **ret)
{
	t_list	*next;
	size_t	nb_nullchars;
	...
	free_args(args);
	free(output);
	*ret = ft_lst_to_str(next);
	return (ft_lst_freestr(next) + nb_nullchars);
}
...
int			ft_asprintf(char **ret, const char *format, ...)
{
	...
	va_end(ap);
	return (end_ft_printf(output, args, ret));
}
