...
/*   Updated: 2016/01/02 14:41:23 by sguillia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	end_ft_printf(t_list *output, t_list *args, int fd)
{
	t_list *next;
	size_t nb_nullchars;
	...
	free_args(args);
	free(output);
	return (ft_lstprintstr_fd(next, fd) + nb_nullchars);
}
...
int			ft_printf_fd(int fd, const char *format, ...)
{
	...
	va_end(ap);
	return (end_ft_printf(output, args, fd));
}
